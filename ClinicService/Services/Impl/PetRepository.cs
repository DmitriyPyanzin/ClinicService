    ...
    public class PetRepository : IPetRepository
    {
        ...
        public int Create(Pet item)
        {
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                command.CommandText = "INSERT INTO pets(ClientId, Name, Type, Breed, Birthday) VALUES(@ClientId, @Name, @Type, @Breed, @Birthday)";
                command.Parameters.AddWithValue("@ClientId", item.ClientId);
                command.Parameters.AddWithValue("@Name", item.Name);
                command.Parameters.AddWithValue("@Type", item.Type);
                command.Parameters.AddWithValue("@Breed", item.Breed);
        ...
        public int Update(Pet item)
        {
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                SqliteCommand command = connection.CreateCommand();
                command.CommandText = "UPDATE pets SET ClientId = @ClientId, Name = @Name, Type = @Type, Breed = @Breed, Birthday = @Birthday" +
                    " WHERE PetId = @PetId";
